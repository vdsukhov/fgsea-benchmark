...
kegg_res <- bplapply(seq_len(nperm), function(i){
    ...
    ora_res <- runEA(de_res, methods = "ora", gs = kegg.gs, perm=0)
    ora_sigsets <- get_sig_fraction(ora_res)
    return(ora_sigsets)
...
saveRDS(kegg_res, file = paste0(path_prefix, "ora.rds"))
...
gobp_res <- bplapply(seq_len(nperm), function(i){
    ...
    se$GROUP <- c(rep(0, group_size), rep(1, group_size))
    de_res <- runDE(list(se), de.method = "limma")
    ora_res <- runEA(de_res, methods = "ora", gs = gobp.gs, perm=0)
    ora_sigsets <- get_sig_fraction(ora_res)
    return(ora_sigsets)
}, BPPARAM = MulticoreParam(detectCores() - 1))
...
saveRDS(gobp_res, file = paste0(path_prefix, "ora.rds"))
